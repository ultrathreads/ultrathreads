...
// Init 注册前台 API 路由
func (h *Handler) initAppRoutes(api *gin.RouterGroup) {
	svc := h.services
	caches := h.caches

	// --- 创建所有 Handler 实例 ---
	siteHandler := NewSiteHandler(svc.Setting, svc.Appinfo, svc.UserReadState, caches.Stat)
	authHandler := NewAuthHandler(svc.User)
	oauthHandler := &OAuthHandler{}
	captchaHandler := &CaptchaHandler{}
	postHandler := NewPostHandler(svc.Post, svc.User, svc.PostLike, svc.Favorite)
	nodeHandler := NewNodeHandler(svc.Node, svc.UserReadState)
	tagHandler := NewTagHandler(svc.Tag, caches.Tag)
	articleHandler := NewArticleHandler(svc.Article, svc.Favorite, caches.ArticleTag, caches.Tag)
	userHandler := NewUserHandler(svc.User, svc.Post, svc.UserScore, svc.UserScoreLog, svc.Notification, svc.Favorite, svc.Article, svc.UserWatch, svc.Rbac)
	linkHandler := NewLinkHandler(svc.Link)
	favoriteHandler := NewFavoriteHandler(svc.Favorite)
